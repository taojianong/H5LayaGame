...
class ObjectPool implements IDispose{
	...
	public static recoverObject( item:any ):void{
		...
		if( !item ) return null;
		let clsName:string = typeof item;//egret.getQualifiedClassName( item );
		let pool:ObjectPool = ObjectPool._pool[ clsName ];
		if( pool == null ){
			let cls:any = Laya.ClassUtils.getClass( clsName );// egret.getDefinitionByName( clsName );
			pool = new ObjectPool( cls );
			ObjectPool._pool[ clsName ] = pool;
		}
	...
	public getObject( args:Array<any> ):any{
		...
		if( Global.is( obj , "IPool" ) ){//egret.is( obj , "IPool" ) 
	...
	public dispose():void{
		...
		while( this._pool && this._pool.length > 0 ){
			...
			if( Global.is( obj , "IDispose") ){//egret.is( obj , "IDispose" )
